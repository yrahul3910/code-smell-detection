...
def get_xy(projectname, mn_maxlen, tokenizer,embedding_matrix):
	path='../Database/'
	...
	train_size = int(.8 * len(labels))
	mn_train, mn_test = mn_datas[:train_size], mn_datas[train_size:]
	metrics_train, metrics_test = metrics_datas[:train_size], metrics_datas[train_size:]
	labels_train, labels_test = labels[:train_size], labels[train_size:]

	x_train = []
	x_train.append(mn_train)
	x_train.append(metrics_train)

	x_test = [mn_test, metrics_test]
...
def get_embedding_matrix(all_word_index, model_path, dim):
	...
	#w2v_model = np.load(model_path, allow_pickle=True)
